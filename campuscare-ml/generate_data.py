...
def make_generator(items, issues, locations, templates, category):
    complaints = []
    seen = set()
    attempts = 0
    while len(complaints) < 180 and attempts < 10000:
        attempts += 1
        ...
        if "{action}" in template and "actions" in globals(): kwargs["action"] = random.choice(actions)
        if "{feeling}" in template and "feelings" in globals(): kwargs["feeling"] = random.choice(feelings)
        
        try:
            text = template.format(**kwargs)
        except KeyError as e:
            # If a special key like {action} is used in a specific generator, we handle it inline.
            pass
    ...
    # If we couldn't generate 180, append variations with random numbers to ensure count
    while len(complaints) < 180:
        base, _, prio = random.choice(complaints)
        text = base + f" (Report ID: {random.randint(1000, 9999)})"
        if text not in seen:
            seen.add(text)
            complaints.append((text, category, prio))
...
def generate_civil():
    items = ["bench", "door", "windowpane", "chair", "desk", "wall", "ceiling", "roof", "staircase", "elevator door", "whiteboard", "pathway", "shed", "gym equipment", "water cooler stand", "blackboard", "bookshelf", "podium"]
    issues = ["broken", "damaged", "cracked", "shattered", "leaking", "jammed", "scratched", "rusted", "collapsed", "peeling", "wobbly", "unstable", "falling apart"]
    locations = ["classroom 101", "classroom 302", "the library", "block B corridor", "the seminar hall", "the auditorium", "hostel room", "the faculty room", "the canteen", "the main gate", "the study room", "the guest house", "the lab", "the sports complex"]
    
    templates = [
        ("The {item} in {location} is {issue} and needs immediate replacement.", "Medium"),
        ("There is a {issue} {item} near {location} that poses a risk.", "High"),
        ("{location} has a {issue} {item} which needs fixing.", "Medium"),
        ...
        ("A {item} fell and is now {issue} in {location}.", "High"),
        ...
        ("Huge problem in {location}: {item} is totally {issue}.", "High"),
        ("We need a new {item} in {location} because the old one is {issue}.", "Low"),
        ("The {item} structure at {location} looks {issue} and might fall.", "Urgent"),
...
def generate_electrical():
    items = ["fan", "air conditioner", "tube light", "bulb", "switchboard", "socket", "power outlet", "projector", "water heater", "microwave", "elevator", "main gate motor", "fire alarm", "generator", "exhaust fan", "street light"]
    issues = ["not working", "sparking", "flickering", "making a loud noise", "fused", "completely dead", "giving electric shocks", "short-circuited", "malfunctioning", "unresponsive", "smoking", "burnt out"]
    locations = ["my hostel room", "the corridor", "lecture hall 2", "the server room", "the library", "the washroom", "the pantry", "the auditorium", "block C", "the parking lot", "the main entrance"]
...
def generate_sanitation():
    items = ["washroom", "toilet", "drainage", "sewage pipe", "water cooler", "dustbin", "sink", "shower", "drinking water filter", "garbage dump", "urinal", "hallway", "lobby", "staircase"]
    issues = ["dirty", "leaking", "blocked", "overflowing", "flooded", "smelly", "clogged", "unusable", "dispensing brown water", "full of mosquitoes", "not cleaned", "filthy", "covered in mud"]
    locations = ["the boys hostel", "the girls hostel", "block A", "the canteen area", "the academic block", "the ground floor", "the library", "the sports complex"]
...
def generate_it():
    items = ["WiFi", "internet", "college website", "student portal", "ERP login page", "lab computers", "department database", "smart board", "printer", "VPN", "biometric attendance", "campus app"]
    issues = ["very slow", "completely down", "crashing", "throwing a 500 error", "hacked", "infected with ransomware", "unresponsive", "locked out", "disconnecting frequently", "showing wrong data", "having huge latency"]
...
def generate_mess():
    items = ["food", "rice", "curry", "milk", "tea", "drinking glasses", "plates", "dinner", "breakfast", "lunch", "soup", "kitchen counter", "diet plan", "canteen food"]
    issues = ["too spicy", "completely raw", "undercooked", "sour and spoiled", "unwashed and oily", "tasteless", "served very late", "cold", "unhygienic", "overpriced", "full of insects"]
    locations = ["the mess"]
...
def generate_disciplinary():
    items = ["phone", "wallet", "laptop", "gold chain", "power bank", "spectacles", "cycle", "bag", "charger", "jewellery", "watch", "headphones"]
    actions = ["stolen", "missing", "taken", "lost", "snatched"]
...
def generate_personal():
    issues = ["headache", "fever", "stomach pain", "anxiety", "depression", "dizziness", "panic attack", "allergic reaction", "back pain", "bleeding", "vomiting", "chest pain", "nausea"]
    actions = ["fainted", "injured my leg", "broke my arm", "cut my finger", "twisted my ankle", "collapsed", "slipped", "got burnt"]
    feelings = ["lonely and homesick", "stressed about my exams", "suicidal", "extremely overwhelmed", "depressed"]
    ...
    seen = set()
    attempts = 0
    while len(complaints) < 180 and attempts < 5000:
        attempts += 1
        template, default_prio = random.choice(templates)
        kwargs = {}
        if "{issue}" in template: kwargs["issue"] = random.choice(issues)
    ...
    while len(complaints) < 180:
        base, prio = random.choice(complaints)
        text = base + f" (Report ID: {random.randint(100, 9999)})"
        if text not in seen:
            seen.add(text)
            complaints.append((text, "Personal", prio))
...
def generate_financial():
    items = ["fee payment", "scholarship amount", "refund for hostel fees", "caution money", "stipend for the internship", "financial aid disbursement", "bus transport fee", "mess fee deduction", "semester fee", "library fine", "late fee penalty", "education loan document"]
    issues = ["not reflecting in the system", "delayed by two months", "wrongfully rejected", "showing incorrect amount", "higher than my actual consumption", "severely delayed causing me hardship", "deducted but transaction failed", "showing double payment", "crashing during submission"]
    locations = [""]
