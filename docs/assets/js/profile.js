...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Load Profile
    try {
        const res = await fetch((window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://localhost:5000' : 'https://campuscare-backend-96cn.onrender.com') : 'https://campuscare-backend-96cn.onrender.com') + '/api/auth/profile', {
    ...
    avatarInput.addEventListener('change', async (e) => {
        ...
        try {
            const res = await fetch((window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://localhost:5000' : 'https://campuscare-backend-96cn.onrender.com') : 'https://campuscare-backend-96cn.onrender.com') + '/api/auth/profile-picture', {
                method: 'POST',
...
async function fetchMarMoocs(token, grid, count) {
    try {
        ...
        const res = await fetch((window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://localhost:5000' : 'https://campuscare-backend-96cn.onrender.com') : 'https://campuscare-backend-96cn.onrender.com') + '/api/mar-moocs', {
            headers: { 'Authorization': `Bearer ${token}` }
        });
